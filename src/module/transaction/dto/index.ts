...
export function IsStripePaymentMethodId() {
  return ValidateBy({
    ...
    validator: {
      ...
      defaultMessage: () => 'paymentMethodId must be a valid Stripe payment method ID (e.g., pm_1abc...)',
    },
  });
}
...
function isAboveMinimumAmount(value: any, ctx: any): boolean {
  ...
  const minimum = MINIMUM_AMOUNTS[currency] || 0.50; // Default to $0.50
  ...
  return ValidateBy({
    ...
    validator: {
      ...
      defaultMessage: (ctx: any) => {
        const minimum = (ctx as any).constraints?.minimum || 0.50;
        const currency = (ctx as any).constraints?.currency || 'USD';
