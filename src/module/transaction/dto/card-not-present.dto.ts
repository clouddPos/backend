...
export class CardNotPresentDto {
  ...
  @IsOptional()
  @IsString()
  @Matches(/^(pm_|tok_)\w+$/, { message: 'paymentMethodToken must be a valid Stripe token (pm_* or tok_*)' })
  paymentMethodToken?: string;

  @ApiPropertyOptional({
    example: '4242424242424242',
    description: 'Full card number (16 digits) - requires Stripe raw card data API access',
    minLength: 13,
    maxLength: 19,
  })
  @IsOptional()
  @IsString()
  @Matches(/^\d{13,19}$/, { message: 'cardNumber must be a valid card number (13-19 digits)' })
  cardNumber?: string;
  ...
  validate() {
    if (!this.paymentMethodToken && !this.cardNumber) {
      throw new Error('Either paymentMethodToken or cardNumber must be provided');
    }
    if (this.paymentMethodToken && this.cardNumber) {
