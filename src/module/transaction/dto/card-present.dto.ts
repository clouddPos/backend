...
export class CardPresentDto {
  ...
  @ApiPropertyOptional({
    ...
    description:
      'Card reader token (for modern terminals like Stripe Terminal)',
  })
  @IsOptional()
  @IsString()
  @Matches(/^pm_\w+$/, {
    message:
      'paymentMethodToken must be a valid Stripe payment method ID (e.g., pm_1abc...)',
  })
  ...
  @ApiPropertyOptional({
    example: '789012',
    description:
      '6-digit authorization code from card reader/terminal (for legacy terminals)',
    minLength: 6,
    maxLength: 6,
  })
  @IsOptional()
  ...
  validate() {
    if (!this.paymentMethodToken && !this.authorizationCode) {
      throw new Error(
        'Either paymentMethodToken or authorizationCode must be provided',
      );
    }
    if (this.paymentMethodToken && this.authorizationCode) {
      throw new Error(
        'Cannot provide both paymentMethodToken and authorizationCode',
      );
    }
    return true;
  }
