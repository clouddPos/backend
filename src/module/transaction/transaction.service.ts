...
export class TransactionService {
  ...
  async initiate(dto: InitiateTransactionDto, idempotencyKey?: string, merchantId?: string) {
    ...
    if (dto.transactionPin && (dto.type === TransactionType.ONLINE || dto.type === TransactionType.CRYPTO)) {
  ...
  async updateStatus(
    ...
    if (newStatus === TransactionStatus.SETTLED) {
      this.logger.log(`Triggering notifications and blockchain sync for settled tx ${id}`);
      ...
      if (transaction.merchant?.email) {
        ...
        this.emailService.sendTransactionSuccessEmail(
          transaction.merchant.email,
          id,
          Number(transaction.amount),
          transaction.currency,
          transaction.type,
        ).catch(err => this.logger.error(`Failed to send email: ${err.message}`));
      }
    ...
    } else if (newStatus === TransactionStatus.CAPTURED && updated.authorizationCode) {
  ...
  async advanceStatus(
    ...
    if (TransactionStateMachine.isTerminal(transaction.status)) return transaction;
   ...
   */
  private async checkForDuplicateTransaction(merchantId: string, dto: InitiateTransactionDto) {
    ...
    const similarTransactions = await this.db.transaction.findMany({
      where: {
        ...
        status: {
          in: [TransactionStatus.INITIATED, TransactionStatus.AUTHORIZED, TransactionStatus.SETTLED],
        },
      },
      take: 5,
      orderBy: { createdAt: 'desc' },
