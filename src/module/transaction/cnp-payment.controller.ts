...
`,
  ...
  async processPayment(
    ...
    await this.transactionService['verifyMerchantPin'](merchantId, dto.transactionPin);

    // Generate idempotency key if not provided
    const key = idempotencyKey || `cnp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    const responseData = {
      ...
      last4: paymentMethodDetails?.card?.last4 || dto.cardNumber?.slice(-4) || '****',
