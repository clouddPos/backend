...
export class NowPaymentsService {
   ...
   */
  async createPayment(request: CreatePaymentRequest): Promise<NowPaymentOrder> {
    try {
    ...
    try {
      ...
      const { data } = await firstValueFrom(
        this.httpService.get(
          `${this.apiUrl}/transaction?${queryParams.toString()}`,
          {
            headers: this.headers,
          },
        ),
  ...
  async getTransactionsByOrderId(orderId: string): Promise<any> {
    try {
      ...
      this.logger.error(
        `getTransactionsByOrderId failed: ${error.response?.data?.message || error.message}`,
      );
      return { data: [] };
    }
  }
  ...
  async getAvailableCurrencies(): Promise<any> {
    try {
      ...
      this.logger.error(
        `getAvailableCurrencies failed: ${error.response?.data?.message || error.message}`,
      );
      return { currencies: [] };
    }
  }
  ...
  async getMinimumPaymentAmount(
    ...
    try {
      ...
      this.logger.error(
        `getMinimumPaymentAmount failed: ${error.response?.data?.message || error.message}`,
      );
      return null;
    }
  }
  ...
  async getEstimatedPrice(
    ...
    try {
      ...
      this.logger.error(
        `getEstimatedPrice failed: ${error.response?.data?.message || error.message}`,
      );
      return null;
    }
  }
  ...
  async getAccountBalance(): Promise<any> {
    try {
      ...
      this.logger.error(
        `getAccountBalance failed: ${error.response?.data?.message || error.message}`,
      );
      return null;
    }
  }
  ...
  verifyIpnSignature(payload: string, signature: string): boolean {
    if (!this.ipnSecret) {
      this.logger.warn('IPN secret not configured — skipping verification');
      return true;
    }
   ...
   * Note: Payouts usually require 2FA verification afterwards.
   */
  async createPayout(
    address: string,
    amount: number,
    ...
    try {
      ...
      this.logger.log(`Payout created: ${data.id} for ${amount} ${currency}`);
      return data;
    } catch (error: any) {
      const message = error.response?.data?.message || error.message;
      this.logger.error(`createPayout failed: ${message}`);
      throw new Error(`NowPayments Payout error: ${message}`);
    }
  }

  /**
   * Verify a payout using a caller-provided 2FA TOTP.
   * OTP generation is intentionally handled outside this client.
   */
  async verifyPayout(payoutId: string, twoFactorCode: string): Promise<any> {
    try {
      const { data } = await firstValueFrom(
        this.httpService.post(
          `${this.apiUrl}/payout/${payoutId}/verify`,
          {
      ...
      this.logger.log(`Payout ${payoutId} verified successfully`);
      return data;
    } catch (error: any) {
      const message = error.response?.data?.message || error.message;
      this.logger.error(`verifyPayout failed for ${payoutId}: ${message}`);
      throw new Error(`NowPayments Verification error: ${message}`);
    }
  }
  ...
  async getPayoutStatus(payoutId: string): Promise<any> {
    try {
      const { data } = await firstValueFrom(
        this.httpService.get(`${this.apiUrl}/payout/${payoutId}`, {
          headers: this.headers,
        }),
      );
      return data;
      ...
      const message = error.response?.data?.message || error.message;
      this.logger.error(`getPayoutStatus failed for ${payoutId}: ${message}`);
      throw new Error(`NowPayments error: ${message}`);
    }
  }
}
