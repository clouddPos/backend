...
`,
  ...
  @ApiQuery({
    ...
    required: false,
    example: 'false',
    description: 'Filter for zero-decimal currencies (e.g., JPY, KRW). Use "true" or "false"',
  })
  @ApiResponse({
    status: 200,
  ...
  async getCurrencies(
    ...
    if (zeroDecimal !== undefined) {
      ...
      filtered = filtered.filter((c) => c.decimalPlaces === 0 === isZeroDecimal);
    }
  ...
  @ApiOperation({
    ...
    description: 'Get detailed information about a specific currency by its ISO code',
  })
  @ApiQuery({
    name: 'code',
    required: true,
