...
export class StripeWebhookController {
    ...
    private async handlePaymentSucceeded(event: any, merchantId: string) {
        ...
        const authCode = charge.payment_method_details?.card?.network_authorization_code;

        this.logger.log(`Payment succeeded: ${paymentIntent.id}, Auth Code: ${authCode}`);
        ...
        this.socketGateway.notifyPaymentSettled(merchantId, {
            ...
            cardScheme: charge.payment_method_details?.card?.brand?.toUpperCase() || 'UNKNOWN',
    ...
    private async handlePaymentFailed(event: any, merchantId: string) {
        ...
        this.logger.warn(`Payment failed: ${paymentIntent.id} - ${lastPaymentError?.message}`);
    ...
    private async handleChargebackReceived(event: any, merchantId: string) {
        ...
        this.logger.warn(`Chargeback received: ${dispute.id} for charge ${dispute.charge}`);
