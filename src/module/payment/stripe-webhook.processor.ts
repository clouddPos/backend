...
export class StripeWebhookProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(
      `Processing Stripe event: ${eventType} [${stripeEvent.id}]`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Error processing Stripe event ${stripeEvent.id}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private async handlePaymentIntentSucceeded(paymentIntent: any) {
    ...
    const transaction = await this.db.transaction.findFirst({
      where: {
        OR: [{ gatewayReference }, { id: transactionId }],
      },
    });

    if (!transaction) {
      this.logger.warn(
        `Transaction not found for PaymentIntent: ${gatewayReference}`,
      );
      return;
    }
    ...
    this.logger.log(
      `Transaction ${transaction.id} advanced to SETTLED (Auth: ${authorizationCode})`,
    );
  }
