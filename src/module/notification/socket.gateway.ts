...
const allowedOrigins = process.env.ALLOWED_FRONTEND_URLS?.split(',').map(
  (url) => url.trim(),
) ?? ['http://localhost:3000'];
  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      this.logger.log(
        `POS Client connected to room merchant-default: ${client.id}`,
      );
    } catch (err: any) {
      this.logger.warn(
        `Socket auth error for client ${client.id}: ${err.message}`,
      );
      client.disconnect();
    }
  }
  ...
  notifyTransactionSettled(merchantId: string, payload: any) {
    this.server
      ...
      .emit('transaction.settled', payload);
  ...
  notifyTransactionAuthorized(merchantId: string, payload: any) {
    this.server
      .to(`merchant-${merchantId}`)
      .emit('transaction.authorized', payload);
  ...
  notifyPaymentAuthorized(
    ...
    this.server
      .to(`merchant-${merchantId}`)
      .emit('payment.authorized', payload);
    this.logger.log(
      `Emitted payment.authorized to merchant-${merchantId}: ${payload.transactionId}`,
    );
  }
  ...
  notifyPaymentSettled(
    ...
    this.logger.log(
      `Emitted payment.settled to merchant-${merchantId}: ${payload.transactionId}`,
    );
  }
  ...
  notifyPaymentFailed(
    ...
    this.logger.log(
      `Emitted payment.failed to merchant-${merchantId}: ${payload.transactionId}`,
    );
  }
  ...
  notifyRefundProcessed(
    ...
    this.logger.log(
      `Emitted refund.processed to merchant-${merchantId}: ${payload.transactionId}`,
    );
  }
  ...
  notifyChargebackReceived(
    ...
    this.server
      .to(`merchant-${merchantId}`)
      .emit('chargeback.received', payload);
    this.logger.log(
      `Emitted chargeback.received to merchant-${merchantId}: ${payload.transactionId}`,
    );
  }
