...
export class EmailService {
    ...
    constructor(private configService: ConfigService) {
        this.transporter = nodemailer.createTransport({
            host: this.configService.get<string>('smtp.host') || 'smtp.ethereal.email',
    ...
    async sendTransactionSuccessEmail(to: string, transactionId: string, amount: number, currency: string, method: string) {
        try {
        ...
        `,
            ...
            this.logger.log(`Success email sent for tx ${transactionId} to ${to}. MessageId: ${info.messageId}`);
        } catch (error: any) {
            this.logger.error(`Failed to send email for tx ${transactionId}: ${error.message}`);
        }
    }
}
